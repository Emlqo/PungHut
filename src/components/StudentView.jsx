...
export default function StudentView({ onBack }) {
  ...
  return (
    <div className="p-4 min-h-screen flex flex-col bg-slate-100 text-center font-sans">
            ...
            <div className="grid grid-cols-2 gap-4 flex-1">
              {currentQuiz.options.map((colorText, i) => (
                <button 
                  ...
                  className={`flex flex-col items-center justify-center p-4 rounded-3xl shadow-[0_8px_0_rgba(0,0,0,0.2)] active:translate-y-2 active:shadow-none transition-all ${['bg-red-500','bg-blue-500','bg-amber-400','bg-emerald-500'][i]}`}
                >
                  <span className="text-xl font-bold text-white break-words drop-shadow-md text-center">
                    {colorText}
                  </span>
                </button>
              ))}
